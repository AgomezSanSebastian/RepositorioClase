...
class modelo {
  ...
  public function actuser($datos) {
    ...
    try {
      ...
      $this->conexion->beginTransaction();
      //Definimos la instrucción SQL parametrizada 
      $sql = "UPDATE `usuario` SET `nif`= :nif,`nombre`= :nombre,`apellido1`= :apellido1,`apellido2`= :apellido2,`imagen`= :imagen,`password`= :password,`email`= :email,`telefono`= :telefono,`direccion`= :direccion,`rol`= :rol WHERE id=:id";
      $query = $this->conexion->prepare($sql);
      $query->execute([
        'nif' => $datos["nif"],
        'nombre' => $datos["nombre"],
